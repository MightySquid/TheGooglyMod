...
import net.mcreator.googlymod.procedures.StrangeMatterProcedureProcedure;
import net.mcreator.googlymod.itemgroup.GooglyModItemGroup;
import net.mcreator.googlymod.item.GoogleshItem;
...
public class GoogliteOreBlock extends GooglymodModElements.ModElement {
	@ObjectHolder("googlymod:googlite_ore")
	public static final Block block = null;
	public GoogliteOreBlock(GooglymodModElements instance) {
		super(instance, 3);
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.ROCK).sound(SoundType.STONE).hardnessAndResistance(5f, 14f).setLightLevel(s -> 6).harvestLevel(5)
					.harvestTool(ToolType.PICKAXE).setNeedsPostProcessing((bs, br, bp) -> true).setEmmisiveRendering((bs, br, bp) -> true));
			setRegistryName("googlite_ore");
		}
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(GoogleshItem.block, (int) (1)));
		}
		...
		public boolean removedByPlayer(BlockState state, World world, BlockPos pos, PlayerEntity entity, boolean willHarvest, FluidState fluid) {
			...
			int z = pos.getZ();
			{
				...
				StrangeMatterProcedureProcedure.executeProcedure($_dependencies);
			}
			return retval;
		}
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("googlymod:googly_biome").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		...
		}.withConfiguration(new OreFeatureConfig(new BlockMatchRuleTest(StrangeStoneBlock.block.getDefaultState().getBlock()) {
			public boolean test(BlockState blockAt, Random random) {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == StrangeStoneBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
